...
fn left_window_title_hook(ctx: &mut InlineCtx) {
    unsafe {
        *ctx.registers[1].x.as_mut() = skyline::c_str("Characters\0") as u64;
    }
}
...
fn chara_description_hook(ctx: &mut InlineCtx) {
    ...
    unsafe {
        *ctx.registers[20].x.as_mut() = b"Reach enlightenment. Seek the thighs. Avoid the socks.\0\0".as_ptr() as u64;
    }
